...
normalization：RMSNorm → torch.nnで用いる
FFN：SwiGLU
attention：MHA
"""
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden = torch.matmul(weights, V)"""    #GPU資源の観点からの高速化のために以下の一行に書き換え
...
if __name__ == "__main__":
    ...
    n_head = 12
    n_layer = 20
    ff_dim = 2560 #ゼロから作るDeepLearning６記載のint(embed_dim * 8 / 3)よりも若干多い
    theta = 10000

    # モデルの初期化
    model = OWT_LLM_v0(vocab_size, max_context_len, embed_dim, n_head,
                   n_layer, ff_dim, theta)
    # 動作テスト
    batch_size = 2
    dummy_input = torch.randint(0, vocab_size, (batch_size, max_context_len))
    logits = model(dummy_input)
    print(logits.shape) #torch.size([2, 1024, 50000])